  ...
  if(m!=nrow(designmatrix)){
    ...
    if(m!=length(sampling.var)){
      ...
      if(det(t(designmatrix)%*%designmatrix)==0){
        ...
        # this line below compute the prasad rao estimate of the variance component
	...
	if(p>1){
	  ...
	  prasadrao=(sum((response-mean(response))^2)/(m-1))-(sum(sampling.var)/m)
        ...
        # print((diag(m)-covariate%*%solve(t(covariate)%*%covariate)
        # %*%t(covariate))%*%diag(sampling.var))
        
        
        #print(sum(diag((diag(m)-covariate%*%solve(t(covariate)%*%covariate)
        # %*%t(covariate))%*%diag(sampling.var))))
...
#prasadraoest(response,covariate,sampling.var)
# the arguments of this function are 
# (i)the response a numeric vector
# (ii)the covariate which is a matrix of the covariate values of class as.matrix
# (iii) the sampling.var a vector specifying the variances of the random components
...
# response=c(1,2,3,4,5)
#covariate=diag(5)
# covariate=cbind(c(1,2,4,4,1),c(2,1,3,1,5))
# sampling.var=c(0.5,0.7,0.8,0.4,0.5)

#prasadraoest(response,covariate,sampling.var)
#fayherriot(response,covariate,sampling.var)
  ...
  if(m!=nrow(designmatrix)){
    ...
    if(m!=length(sampling.var)){
      ...
      if(det(t(designmatrix)%*%designmatrix)==0){
        ...
        initial=c(psi,beta)
        #print(log(psi+sampling.var))
        #print(initial)
        #print(mygradient(initial))
        #mle=optim(initial,objectivefn,mygradient,method="L-BFGS-B",lower=0, upper=Inf)$par
        optimize=optim(initial,objectivefn,mygradient,method="BFGS")
        #optimize=optim(initial,objectivefn,method="SANN")
        mle=optimize$par
...
resimaxilikelihood=function(response,designmatrix,sampling.var,maxiter){
  ...
  Pmatrix=function(scalar){
    inversedata.var(scalar)-(inversedata.var(scalar)%*%designmatrix%*%solve(t(designmatrix)%*%inversedata.var(scalar)%*%designmatrix)%*%(t(designmatrix))%*%inversedata.var(scalar))
  }
...
designmatrix=as.matrix(covariate[,1])
  ...
  B=function(vec,estimate){diag(vec/(estimate+vec))}
  ...
  solve(t(designmatrix)%*%diag(1/(psi+sampling.var))%*%designmatrix)%*%t(designmatrix)%*%
    ...
    g2=function(vec,psi,mat){
  (vec/(vec+psi))^2 * diag(mat%*%solve(t(mat)%*%diag(1/(psi+vec))%*%mat)%*%t(mat))
    }
   
    # g3 function for prasad rao
    g3.pr=function(vec,psi){
      (2/length(vec)^2)*(vec^2/(psi+vec)^3)*sum((rep(psi,times=length(vec))+vec)^2)
    }
    
    # g3 function for the fay herrit
    g3.fh=function(vec,psi){
      (2*length(vec))*(vec^2/(psi+vec)^3)*(sum(1/(rep(psi,times=length(vec))+vec)))^(-2)
    }
    
    
    # g3 function for the ml and reml
    g3.other=function(vec,psi){
      2*(vec^2/(psi+vec)^3)*(sum(1/(rep(psi,times=length(vec))+vec)^2))^(-1)
    }
...
# of the variance component. Note that this bias is 0 for the methods PR and REML
  ...
  varcomp.bias.ml=function(vec,psi,mat){
      (1/(sum(1/((psi+vec)^2))))*sum(diag(solve(t(mat)%*%diag(1/(psi+vec))%*%mat)%*%(t(mat)%*%diag(1/(psi+vec)^2)%*%mat)))
      }




   varcomp.bias.fh=function(vec,psi){
      2*(sum((psi+vec)^(-1)))^(-3)*((length(vec)*
                                      sum((psi+vec)^(-2)))-(sum(1/(psi+vec)))^2)
    }
...
# the second order bias correction for different cases
   ...
   bias.ml=function(vec,psi,mat){
      (vec/(psi+vec))^2*(1/(sum(1/(rep(psi,times=length(vec))+vec)^2)))*
        sum(diag(solve(t(mat)%*%diag(1/(psi+
                              vec))%*%mat)%*%(t(mat)%*%diag(1/(psi+vec)^2)%*%mat)))
    }
 


    bias.fh=function(vec,psi){
      2*(vec/(psi+vec))^2*(sum((psi+vec)^(-1)))^(-3)*((length(vec)*
                                      sum((psi+vec)^(-2)))-(sum(1/(psi+vec)))^2)
    }
    if(method=="FH"){
...
#data=data.frame(response=c(1,2,3,4,8),x1=c(1,5,6,4,2),x2=c(5,1,3,2,1),D=c(0.2,0.5,0.1,0.9,1))
#data=data.frame(response=c(1,2,3,4,8),D=c(0.2,0.5,0.1,0.9,1))
#data=data.frame(response=c(1,2,3,4),covariate=c(1,5,6,4))
#data
#attach(data)
#ans=smallareafit(response~D+x1+x2,method="FH")
...
#ans4=smallareafit(response~D+x1+x2,data,method="BAL")
#ans
#ans1
#ans2
#ans3
...
#resimaxilikelihood(response,designmatrix,sampling.var)
#data=data.frame(cbind(response=c(1,2,3,4,7),x1=c(2,1,4,3,6),x2=c(3,1,5,8,3),D=c(0.5,0.2,0.7,0.4,0.2)))
 #smallareafit(response~x1+x2,data,)
